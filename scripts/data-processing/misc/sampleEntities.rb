...
def sampleEntities(triples, size)
   entities = []
   entities += triples.map{|triple| triple[Constants::HEAD]}
   entities += triples.map{|triple| triple[Constants::TAIL]}
   entities = Set.new(entities.uniq().sample(size, random: Random.new(SEED)))

   triples.delete_if{|triple| !entities.include?(triple[Constants::HEAD]) || !entities.include?(triple[Constants::TAIL])}
...
def parseArgs(args)
   if (args.size != 2 || args.map{|arg| arg.gsub('-', '').downcase()}.include?('help'))
      puts "USAGE: ruby #{$0} <data dir> <num entities to sample>"
      puts "   Output will be written in a new directory adjacent to |data dir|, called |data dir|_ENTITY_SAMPLE[N]"
      puts "   Where 'N' is the number of entities sampled (zero padded to 6 digits)."
      exit(1)
   end
   ...
   if (size < 1)
      puts "Number of entities to sample must be at least 1 (found #{size})."
      exit(2)
   end
...
def main(args)
   ...
   outDir = File.absolute_path(dataDir) + "_ENTITY_SAMPLE[#{"%06d" % size}]"
   FileUtils.mkdir_p(outDir)

   puts "Sampling and creating new dataset in: #{outDir}"

   triples = getAllTriples(dataDir)
   triples = sampleEntities(triples, size)
